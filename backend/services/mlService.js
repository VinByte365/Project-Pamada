...
class MLService {
  constructor() {
    ...
    this.maturityConfidenceThreshold = Number(
      process.env.MATURITY_CONFIDENCE_THRESHOLD || 0.2,
    );
  }
  ...
  async analyzeImage(imageBuffer, metadata = {}) {
    try {
      ...
      try {
        ...
        console.warn(
          "Maturity endpoint unavailable, continuing with disease analysis:",
  ...
  async analyzeDisease(imageBuffer, metadata = {}) {
    ...
    // Forward maturity_prediction from ML service so combined confidence propagates
    if (oldData?.maturity_prediction) {
      oldData.maturity_prediction = oldData.maturity_prediction;
    }
    return oldData;
  }
  ...
  async analyzeMaturity(imageBuffer, metadata = {}) {
    ...
    formData.append(
      ...
      String(this.maturityConfidenceThreshold),
    );
  ...
  generateAnalysisResult(mlResults, plant = null) {
    ...
    // If the ML service returned a combined confidence_score, prefer it.
    const combinedConfidence = Number(confidence_score || 0);
    const effectiveConfidence =
      combinedConfidence > 0 ? combinedConfidence : parsedConfidence;
    const invalidConfidence =
      !Number.isFinite(effectiveConfidence) || effectiveConfidence <= 0;
    const noPlantDetected =
      stageLower.includes("no plant detected") || invalidConfidence;
    ...
    const primaryPrediction =
      yolo_predictions && yolo_predictions.length > 0
        ? yolo_predictions[0]
        : { class: "healthy", confidence: 0.5 };
    const diseaseDetectedByKey = Boolean(
    ...
    if (diseaseDetectedByKey || primaryPrediction.class !== "healthy") {
      if (
        ...
        ["high", "medium", "low"].includes(
          String(mlResults.severity).toLowerCase(),
        )
      ) {
        const severityMap = { high: "severe", medium: "moderate", low: "mild" };
        diseaseSeverity =
          severityMap[String(mlResults.severity).toLowerCase()] || "moderate";
      } else if (primaryPrediction.confidence >= 0.8) {
    ...
    const maturityAssessment = this.mapMaturityAssessment(
      maturity_data?.maturity_stage,
      age_estimation?.maturity_assessment,
    );
    const estimatedDays = this.mapEstimatedDaysToHarvest(
      maturity_data?.maturity_stage,
      age_estimation?.estimated_days_to_harvest,
    );
    const stageText = String(maturity_data?.maturity_stage || "").toLowerCase();
    const harvestReady =
      stageText.includes("ready for harvest") || stageText.includes("mature");
    ...
    return {
      ...
      disease_detected:
        diseaseDetectedByKey || primaryPrediction.class !== "healthy",
      disease_severity: diseaseSeverity,
      recommended_action: recommendedAction,
      estimated_days_to_harvest: estimatedDays,
      confidence_score: Number.isFinite(effectiveConfidence)
        ? effectiveConfidence
        : 0.5,
      recommendations: {
        treatment_plan: [],
  ...
  generateRecommendations(diseaseClass, severity, ageEstimation) {
    ...
    if (diseaseClass !== "healthy") {
      ...
      recommendations.treatment_plan.push(
        "Monitor plant closely for improvement",
      );
    }
    ...
    recommendations.preventive_measures.push(
